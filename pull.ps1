...
.DESCRIPTION
    Порядок действий:
        1. git pull --ff-only;
        2. быстрый набор проверок ci/run_local.ps1 (около минуты);
    ...
    Гоняется набор fast: тяжёлые проверки (контроль мутаций R32), приёмочные
    (архив, импорт, экспорт, публикация) и ночные в него не входят — иначе
    проверки перед сборкой ждали бы по семь минут и перестали быть гейтом.
    Приёмка запускается вручную: ci/run_local.ps1 -Suite acceptance.
