...
// --- Перекрёстки: тот же приём проверки имён и самих данных ---
{
  ...
  // Метод обязан быть записан: по нему видно, КАКИМИ признаками получен список.
  // Список строится двумя правилами, потому что перекрёстки в этих данных бывают
  // двух физических видов: пересечение координат и РАЗРЫВ полотна. Разрывов
  // большинство (автор разметил 97 таких вручную, и ни один из них нодировка не
  // находила), поэтому оба правила обязаны быть видны в данных.
  if (typeof payload.method !== "string" || !payload.method.includes("gaps"))
    throw new Error("В junctions.json не записан метод построения по разрывам: " + payload.method);
  if (!payload.method.includes("noding"))
    throw new Error("В junctions.json нет второго метода (пересечения линий): " + payload.method);

  // Регион обязан быть записан: европейская часть не участвует в игровом мире и
  // её присутствие означало бы, что список снова захватил лишнюю половину карты.
  if (typeof payload.region !== "string" || !payload.region.includes("x>="))
    throw new Error("В junctions.json не записан регион построения: " + payload.region);

  const europeMaxX = Number((payload.region.match(/x>=(\d+)/) || [])[1]);
  if (!Number.isFinite(europeMaxX))
    throw new Error("Из региона не читается граница Европы: " + payload.region);

  let inEurope = 0;
  for (let i = 0; i < payload.points.length; i += 2)
    if (payload.points[i] < europeMaxX) inEurope++;
  if (inEurope > 0)
    throw new Error("В списке " + inEurope + " узлов из европейской части: " +
      "она не участвует в игровом мире и должна быть исключена.");

  // Распределение по ПРАВИЛАМ: по нему видно, какое правило дало узлы. Без
  // правила «разрыв» список снова стал бы только пересечениями линий.
  if (!payload.rules || typeof payload.rules !== "object")
    throw new Error("В junctions.json нет разбивки узлов по правилам построения.");
  if (!(payload.rules.converge > 0))
    throw new Error("Правило «сходящиеся разрывы» не дало ни одного узла: " +
      JSON.stringify(payload.rules));

  // Направления в ОКРУЖЕНИИ ячейки. Без этого шага теряются съезды к перекрытым
  // дорогам: третье направление голосует в соседнюю ячейку, и каждая по
  // отдельности даёт только два. Автор называет такие съезды самыми ценными.
  if (typeof payload.method !== "string" || !payload.method.includes("window"))
    throw new Error("В методе нет шага «направления в окружении ячейки»: " +
      "он ловит третье направление, проголосовавшее в соседнюю ячейку.");
  if (!(payload.rules.convergeWindow > 0))
    throw new Error("Правило «направления в окружении» не дало ни одного узла: " +
      JSON.stringify(payload.rules));

  // Политика по обрывам обязана быть ЗАПИСАНА в данные. То, что в геометрии
  // выглядит тупиком, в игре — перекрытый от проезда съезд: дизайнер карты
  // намеренно достраивает все перекрёстки, а проезд закрывает заглушкой, за
  // которой стоит муляж. Заехать нельзя, но съезд существует и как квестовая
  // локация ценен, поэтому автор просил их НЕ отсекать и не бояться числа узлов.
  if (typeof payload.deadEndPolicy !== "string" || !payload.deadEndPolicy.includes("обрывы"))
    throw new Error("В junctions.json не записана политика по обрывам полотна: " +
      "без неё непонятно, что перекрытые съезды сохраняются намеренно.");
  if (!/сохраняются/.test(payload.deadEndPolicy))
    throw new Error("Политика по обрывам не говорит, что они СОХРАНЯЮТСЯ: " +
      payload.deadEndPolicy);
  ...
  // Нижняя граница по правилу разрывов. Автор разметил 97 настоящих перекрёстков
  // глазами, и ни один из них прежний поиск (пересечения линий) не находил — все
  // они именно РАЗРЫВЫ полотна. Правило «сходящиеся разрывы» обязано давать
  // подавляющее большинство узлов: если оно ломается, список вырождается обратно.
  const converge = payload.rules.converge || 0;
  if (converge < payload.junctionCount * 0.5)
    throw new Error("Правило «сходящиеся разрывы» дало всего " + converge + " из " +
      payload.junctionCount + " узлов: похоже, поиск снова смотрит только на " +
      "пересечения линий, а полотно в этих данных прерывается.");
  ...
  // подскакивает в разы. Настоящий мир даёт 3275 узлов.
  //
  // Граница ЩЕДРАЯ намеренно: автор просил НЕ бояться большого числа узлов —
  // лишние он уберёт вручную, а пропущенный съезд на дальней трассе потеряется
  // навсегда. Порог ловит только всплеск от несхлопнутых полос (он даёт десятки
  // тысяч), а не естественный рост от сохранённых обрывов.
  ...
  // объединения направлений каждый поворот даёт лишние ветки, и простые узлы
  // превращаются в «сложные». Замеры на этом же наборе: со схлопыванием сложных
  // узлов 1.2%, без схлопывания — 25.3%. Граница 5% лежит между ними.
  let complex = 0;
  for (const [branchCount, count] of branches) if (branchCount >= 5) complex += count;
  const complexShare = complex / total;

  if (complexShare > 0.05)
