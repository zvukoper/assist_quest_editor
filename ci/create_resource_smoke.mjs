...
// Создание мира не открывает его на живом окне — говорится прямо.
check(/смена мира в открытом окне пока не реализована/.test(mainForm),
  "Создание мира молча обещает переход в него, хотя он не выполняется.");
check(/Кампания отключена/.test(mainForm),
  "Не сказано, что новая кампания отключена: автор будет искать её в симуляции.");
