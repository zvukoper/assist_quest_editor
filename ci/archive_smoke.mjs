...
// Распаковка обязана ВОССТАНАВЛИВАТЬ каталоги, а не пропускать записи с
// завершающим слэшем. Асимметрия здесь уже была: упаковщик писал пустые папки,
// распаковщик их выбрасывал, и мир после «выгрузил → импортировал обратно»
// терял Saves — папку, которую ждёт автосохранение.
check(/Directory\.CreateDirectory\(folderTarget\)/.test(unpackBody),
  "Распаковка обязана создавать каталоги из записей архива: иначе пустые папки теряются.");

// Импорт идёт через временную папку: прерванный импорт не оставляет
// полураспакованный мир, который выглядит рабочим.
//
// Перенос на место идёт через StagedFolderMover, а НЕ безусловным
// Directory.Move. Это не «стилистика»: временная папка лежит на системном
// диске, а каталог миров — в Документах, которые часто перенесены на другой
// диск. Прежняя проверка требовала наличия `Directory.Move(staging, target)`,
// то есть ЗАКРЕПЛЯЛА дефектный приём, и импорт падал у автора всегда
// («Move will not work across volumes»), оставаясь при этом «зелёным» в CI.
check(/aq-import-/.test(store) && /StagedFolderMover\.IntoPlace\(/.test(store),
  "Импорт должен распаковывать во временную папку и ставить её на место через StagedFolderMover.");
check(!/Directory\.Move\(staging/.test(store),
  "Безусловный Directory.Move временной папки запрещён: он не работает между томами.");

// Отступление к копированию обязано быть НАСТОЯЩИМ: правило решает по корню
// пути, а сам перенос копирует содержимое и лишь затем переименовывает — иначе
// каталог миров увидел бы частично скопированный ресурс.
const mover = fs.readFileSync(
  path.join(root, "src", "AssistQuestEditor.App", "Core", "StagedFolderMover.cs"), "utf8");
check(/CanRenameIntoPlace/.test(mover),
  "Перенос обязан проверять, возможно ли переименование на этом томе.");
check(/Directory\.Move\(partial, targetFolder\)/.test(mover),
  "При копировании последним шагом обязано быть переименование: только оно не показывает частично скопированный ресурс.");
check(/OrderForCopy/.test(mover),
  "Файл-определитель ресурса обязан копироваться последним: иначе стор увидит пустой ресурс.");
