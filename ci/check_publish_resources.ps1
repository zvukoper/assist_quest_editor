...
# лежат папка ресурсов и эти два файла. Любой третий файл — остаток прошлой
# сборки, который легко принять за данные. Список обязан идти в ногу со всеми
# отчётами: иначе проверка падает на исправной сборке. Папка data исключается
# по имени: у каталога Extension пуст, и без этой проверки он сам попадал бы в
# «неожиданные файлы» (так проверка один раз и упала на исправной сборке).
$allowedExtra = @('data-verify-report.txt', 'demo-world-report.txt')
