...
//
// ОБРЫВЫ НЕ ОТСЕКАЮТСЯ. То, что в геометрии выглядит тупиком, в игре обычно
// НЕ тупик: дизайнер карты намеренно достраивает все перекрёстки для
// правдоподобности, а перекрытый от проезда участок закрывает заглушкой, за
// которой стоит муляж населённых пунктов. Заехать туда нельзя, но САМ СЪЕЗД
// существует и проходит в основном по дальним трассам — как квестовая локация он
// ценен. Поэтому ни правило 1, ни правило 2 не отбрасывают ветки, ведущие в
// обрыв, и верхняя граница числа узлов в проверке взята очень щедрой: лучше
// отдать лишние узлы автору на ручную проверку, чем потерять съезд.
//
// Проверка на 97 узлах, размеченных автором вручную: метод B находит 97 (100%)
// в радиусе 30 м, медиана расхождения 8.8 м. Метод A не находил НИ ОДНОГО из них —
// поэтому он оставлен только как дополнение для настоящих пересечений координат.
//
...
/**
 * Радиус, в котором считаются направления ВОКРУГ ячейки голосования, метры.
 *
 * Ячейка 20 м — только копилка голосов, и третье направление может проголосовать
 * в СОСЕДНЮЮ ячейку: тогда каждая по отдельности даёт 2 направления, а вместе 3.
 * Так выглядит съезд к перекрытой дороге — сквозная дорога даёт свои два
 * направления, а обрыв боковой улицы смотрит ОТ перекрёстка (в сторону муляжа) и
 * голосует дальше вдоль себя, попадая в соседнюю ячейку.
 */
const VOTE_WINDOW = 35;

/**
 * Схлопывание направлений в правиле окружения — тоньше, чем DIR_MERGE_DEG.
 * В окне 35 м собираются обрывы с обеих сторон перекрёстка, и при схеме 30°
 * противоположные направления сквозной дороги (4° и -176°) сливаются в одно.
 */
const WINDOW_MERGE_DEG = 20;
...
function detectGaps(segments) {
  ...
  const mergeAnglesFor = (angles, deg) => {
    const mergeRad = (deg * Math.PI) / 180;
    const uniq = [];
  ...
  const mergeAngles = (angles) => mergeAnglesFor(angles, DIR_MERGE_DEG);
  ...
  // --- Правило 1б: направления в ОКРЕСТНОСТИ ячейки ---
  //
  // Ячейка голосования нужна только как копилка: она отвечает на вопрос «где
  // вообще есть движение разрывов», но НЕ «сколько направлений в этом месте».
  // Третье направление может проголосовать в соседнюю ячейку, и тогда каждая
  // ячейка по отдельности даёт два направления, а место остаётся перекрёстком.
  //
  // Так выглядит съезд к перекрытой дороге: сквозная дорога даёт свои два
  // направления, а обрыв боковой улицы смотрит ОТ перекрёстка (в сторону муляжа)
  // и потому голосует дальше вдоль себя. Именно эти съезды на дальних трассах
  // автор называет самыми ценными, поэтому терять их нельзя.
  const endGrid = new Map();
  const END_CELL = 30;
  for (const e of dangling) {
    const k = `${Math.floor(e.x / END_CELL)}|${Math.floor(e.z / END_CELL)}`;
    if (!endGrid.has(k)) endGrid.set(k, []);
    endGrid.get(k).push(e);
  }
  const endsNear = (px, pz, radius) => {
    const reach = Math.ceil(radius / END_CELL) + 1;
    const found = [];
    const cx = Math.floor(px / END_CELL), cz = Math.floor(pz / END_CELL);
    for (let ix = cx - reach; ix <= cx + reach; ix++)
      for (let iz = cz - reach; iz <= cz + reach; iz++)
        for (const e of endGrid.get(`${ix}|${iz}`) || [])
          if (Math.hypot(e.x - px, e.z - pz) <= radius) found.push(e);
    return found;
  };
  ...
  for (const k of votes.keys()) {
    const [cx, cz] = k.split("|").map(Number);
    const px = (cx + 0.5) * VOTE_CELL, pz = (cz + 0.5) * VOTE_CELL;
    const ends = endsNear(px, pz, VOTE_WINDOW);
    if (ends.length < VOTE_MIN_DIRS) continue;
    const dirs = mergeAnglesFor(ends.map(e => e.angle), WINDOW_MERGE_DEG);
    if (dirs.length < VOTE_MIN_DIRS) continue;

    // Узел ставится в ЦЕНТР МАСС собранных обрывов: он точнее центра ячейки,
    // потому что обрывы лежат по разные стороны перекрёстка и уравновешивают друг
    // друга, а ячейка привязана к своей сетке.
    windowed.push({
      x: ends.reduce((s, e) => s + e.x, 0) / ends.length,
      z: ends.reduce((s, e) => s + e.z, 0) / ends.length,
      branches: dirs.length,
      rule: "convergeWindow"
    });
  }
  console.log(`Окружения ячеек (${VOTE_WINDOW} м): ${windowed.length} мест`);
  ...
  // Порядок важен: сначала самые ТОЧНЫЕ места. Ячейка голосования привязана к
  // сетке и потому смещена не более чем на половину ячейки; центр масс окна
  // зависит от того, какие обрывы попали в радиус. При равном числе веток
  // дедупликация 40 м сохранила бы то, что пришло первым, поэтому точные идут
  // первыми — иначе расширение окна ухудшает точность (замерено: макс.
  // расхождение с разметкой автора росло с 24 до 29 м).
  for (const p of converging.sort((a, b) => b.branches - a.branches)) push(p);
  for (const p of windowed.sort((a, b) => b.branches - a.branches)) push(p);
...
if (method === "gaps" || method === "both") {
  ...
  // Порядок в метке — от грубого к тонкому: сначала копилка голосов, затем
  // направления в окружении (ловит третье направление в соседней ячейке) и
  // примыкание по лучу.
  methodLabels.push(`gaps:converge${VOTE_MIN}-${VOTE_MAX}m>=${VOTE_MIN_DIRS}dirs` +
    `+window${VOTE_WINDOW}m/${WINDOW_MERGE_DEG}deg` +
...
const payload = {
  ...
  // Политика по обрывам записана в данные намеренно: то, что выглядит тупиком,
  // в игре — перекрытый от проезда съезд, и автор просил их НЕ отсекать.
  deadEndPolicy: "обрывы сохраняются: перекрытые съезды ценны как квестовые локации",
