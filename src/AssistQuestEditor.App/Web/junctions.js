...
(() => {
  ...
  let savedExcluded = [];      // [{x,z}] — исключения, УЖЕ записанные в файл
  ...
  /**
   * Исключённый узел, ещё НЕ записанный в файл — красный.
   *
   * Красный именно у несохранённых: автор должен видеть, что уже вычищено в
   * текущем проходе, но пока не записано, — по нему можно передумать и вернуть
   * узел. Без этого все точки жёлтые, и запутаться очень легко.
   */
  const EXCLUDED_COLOR = "#ff4d4d";

  /**
   * Исключённый узел, УЖЕ записанный в файл — серый.
   *
   * Серый означает «решено окончательно»: при следующем открытии окна автор
   * видит, что́ он вычистил ранее, и не исключает те же узлы повторно.
   *
   * Светло-серый, а НЕ тёмный: дороги рисуются `rgba(118,130,146,.85)` и после
   * наложения на фон дают ровно (102,113,127) — тёмный серый вроде #66707c давал
   * разницу (0,1,3), то есть был НЕОТЛИЧИМ от дорог. Такой узел невозможно было
   * бы ни увидеть глазами, ни отличить по пикселям в проверке (проверка ловила
   * бы пиксели дорог вместо точек). Здесь разница с дорогой — около 51 по каналу.
   */
  const EXCLUDED_SAVED_COLOR = "#9aa4b2";
  ...
  function drawJunctions(ctx) {
    ...
    junctions.forEach((junction, index) => {
      // Исключённые узлы рисуются ОТДЕЛЬНО, из списка исключений, а не отсюда.
      // Причина: после сохранения Host убирает их из присланного списка, и здесь
      // их просто нет — а показать надо, иначе автор не увидит, что́ уже вычищено.
      // Пропуск здесь защищает от двойной отрисовки свежих исключений.
      if (isFreshlyExcluded(junction) || isSavedExcluded(junction)) return;
      ...
      // прежде чем нажмёт. Крупный маркер, а не тонкое кольцо — на плотной карте
      // города тонкое кольцо теряется среди соседних узлов.
      if (isHovered) {
        ctx.save();
    ...
    drawExcludedPoints(ctx);
    drawAddedPoints(ctx);
  }

  /**
   * Исключённые узлы: КРАСНЫЕ, пока не записаны, и СЕРЫЕ после записи.
   *
   * Три состояния точки, и каждое означает своё:
   *  - жёлтая — найдена, ещё не проверена;
   *  - красная — исключена в текущем проходе, но НЕ сохранена: по ней автор ещё
   *    может передумать кнопкой «Вернуть»;
   *  - серая — исключена и записана в файл: решено окончательно, при следующем
   *    открытии окна автор сразу видит, что́ он вычистил ранее.
   *
   * Рисуются из СПИСКА ИСКЛЮЧЕНИЙ, а не из показанного списка узлов: после
   * сохранения Host убирает исключённые из присланных узлов, и вывести их
   * координаты из чего-либо другого невозможно.
   */
  function drawExcludedPoints(ctx) {
    ...
    for (const point of excluded) {
      ...
      if (screen.x < -20 || screen.y < -20) continue;
      if (screen.x > size.width + 20 || screen.y > size.height + 20) continue;

      // Свежее исключение важнее сохранённого: по красному автор ещё может
      // передумать, и оно обязано бросаться в глаза.
      const fresh = isFreshlyExcluded(point);
      const stale = isSavedExcluded(point);

      ctx.save();
      ctx.beginPath();
      ctx.fillStyle = fresh ? EXCLUDED_COLOR : stale ? EXCLUDED_SAVED_COLOR : ACCENT;
      ctx.arc(screen.x, screen.y, 5, 0, Math.PI * 2);
      ctx.fill();

      // Крестик внутри: цвет на мелком масштабе и на снимке различается плохо, а
      // форма — нет. Заодно отличает исключённую точку от жёлтой на слабом экране.
      ctx.strokeStyle = "#0d1014";
      ctx.lineWidth = 1.4;
      ctx.beginPath();
      ctx.moveTo(screen.x - 2.6, screen.y - 2.6);
      ctx.lineTo(screen.x + 2.6, screen.y + 2.6);
      ctx.moveTo(screen.x + 2.6, screen.y - 2.6);
      ctx.lineTo(screen.x - 2.6, screen.y + 2.6);
      ctx.stroke();
      ctx.restore();
  ...
  /**
   * Исключён ли узел в ТЕКУЩЕМ проходе, но ещё НЕ записан в файл.
   *
   * Различие «исключён сейчас» и «исключён ранее» — это различие между красным и
   * серым, и оно определяется ВЫЧИТАНИЕМ: `excluded` содержит всё, а
   * `savedExcluded` — только записанное ранее. Совпавшее с сохранённым красным не
   * станет, по нему автор уже принял решение в прошлый раз.
   */
  function isFreshlyExcluded(junction) {
    const inExcluded = excluded.some(point =>
      Math.hypot(point.x - junction.x, point.z - junction.z) <= MATCH_RADIUS);

    return inExcluded && !isSavedExcluded(junction);
  }

  /** Исключён ли узел В ФАЙЛЕ (то есть был исключён до этого прохода). */
  function isSavedExcluded(junction) {
    return savedExcluded.some(point =>
      Math.hypot(point.x - junction.x, point.z - junction.z) <= MATCH_RADIUS);
  }
  ...
  function toggleAddPoint(px, py) {
    ...
    //
    // Но НЕ рядом с исключённым: вычищенное кольцо стоит сплошняком из серых
    // точек, и именно туда автор и хочет поставить ОДИН зелёный узел для квеста.
    // Проверка «нет ли рядом жёлтого» без этого исключения запрещала бы главный
    // сценарий работы, для которого приоритет добавленного и заведён.
    const nearDetected = junctions.some(j =>
      Math.hypot(j.x - world.x, j.z - world.z) <= 10 &&
      !isFreshlyExcluded(j) && !isSavedExcluded(j));
  ...
  canvas?.addEventListener("pointerup", event => {
    ...
    // Клик по ИСКЛЮЧЁННОМУ узлу не выбирает его, а ДОБАВЛЯЕТ здесь свой.
    //
    // Так выглядит главный сценарий автора: кольцо вычищено, и в вычищенном месте
    // надо поставить ОДИН узел для квеста. Исключённый узел остаётся в списке до
    // сохранения (чтобы «Вернуть» работал без Host), поэтому без этой ветки клик
    // попадал бы в него и переводил в выбор, а не создавал перекрёсток.
    if (index >= 0 && (isFreshlyExcluded(junctions[index]) || isSavedExcluded(junctions[index]))) {
      toggleAddPoint(px, py);
      return;
    }
  ...
  function applyReview(data) {
    ...
    // Всё, что пришло от Host, УЖЕ записано в файл: это исключения прошлых
    // проходов, и они рисуются серыми. Свежие исключения текущего прохода
    // добавляются к `excluded` поверх и рисуются красными.
    //
    // Копия, а не ссылка: `excluded` дальше меняется, и общий массив сделал бы
    // «сохранённые» равными «текущим» — серые точки мгновенно стали бы красными.
    savedExcluded = excluded.map(point => ({ x: point.x, z: point.z }));
  ...
  function handleMessage(event) {
    ...
    if (data.type === "junction_review_saved") {
      // Красные точки становятся СЕРЫМИ: правки записаны, решение принято
      // окончательно. Без этого они оставались бы красными до перезапуска окна и
      // показывали бы «несохранённое» там, где уже сохранено.
      //
      // Обновляется и `savedExcluded`, и сам список: следующее исключение
      // обязано отличаться от только что записанных.
      savedExcluded = excluded.map(point => ({ x: point.x, z: point.z }));
  ...
  window.__assistJunctionReview = {
    ...
    getState: () => ({
      ...
      // Состояние цветов: сколько исключённых ещё не записано (красные) и
      // сколько уже в файле (серые). Проверка обязана видеть ИМЕННО цвета,
      // иначе подмена «всё красное» или «всё серое» прошла бы незамеченной.
      freshExcluded: excluded.filter(point => isFreshlyExcluded(point)).length,
      savedExcluded: savedExcluded.length
    })
  };
