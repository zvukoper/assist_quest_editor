(() => {
  ...
  const handleWebviewMessage = event => {
    ...
    if (data?.type === "quest_sceneGraph") {
      ...
      if (data.selectedNodeId) selectedSceneNodeId = data.selectedNodeId;
      ...
      if (!selectedSceneNodeId && sceneGraph.nodes.length) selectedSceneNodeId = sceneGraph.nodes[0].nodeId;
      if (currentId() === "sceneGraph") render();
    ...
    if (data?.type === "runtime_state") {
      ...
      if (currentId() === "sceneGraph") updateSceneVisuals();
      return;
    }
  ...
  document.addEventListener("keydown", event => {
    ...
    if (event.code === "Space") {
      sceneSpaceDown = true;
      if (currentId() === "sceneGraph") event.preventDefault();
    }

    if (
      currentId() === "sceneGraph" &&
      event.key === "Delete" &&
      !isTextEntryTarget(event.target) &&
      selectedSceneNodeId
    ) {
      if (requestSceneNodeDelete(selectedSceneNodeId)) {
        event.preventDefault();
      }
      return;
    }

    if (currentId() !== "sceneGraph" || !(event.ctrlKey || event.metaKey)) return;
  ...
  render();
  window.__assistQuestSceneGraphRuntime = {
