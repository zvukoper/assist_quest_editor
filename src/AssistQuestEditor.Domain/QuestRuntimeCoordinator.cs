...
public sealed class QuestRuntimeCoordinator : IQuestRuntimeController
{
    ...
    {
        // Остановка сбрасывает ускорение времени. Ускорение — это РЕЖИМ ИГРЫ
        // (сколько игрового времени идёт за секунду), а не часть мира: после
        // остановки оно ничего не значит, и оставленное значение «залипало» —
        // часы оставались оранжевыми с кратностью ×20, хотя симуляции уже нет.
        //
        // Сброс делается ЗДЕСЬ, а не в обработчике кнопки: «стоп» приходит и от
        // кнопки, и от закрытия окна, и от смены мира — в ветке кнопки значение
    ...
    public void Reset()
    {
        ...
        // Ускорение сбрасывается вместе с прохождением: это состояние РЕЖИМА, и
        // после сброса оно не имеет смысла. Ускорение хранится только в памяти —
        // ни в мире, ни в ручных сохранениях его нет, — так что «сбросить" его
        // можно ровно здесь.
        SetSimulationSpeed(1d);
