...
Текущая рабочая версия: 1.0.40.181-WORLD-CAMPAIGN-AUTHORING-R29.

Последние рабочие вехи:
- 1.0.40.181 — мир и кампания стали полноценными ресурсами: `[МИР][КАМПАНИЯ][меню]`
  в шапке главного окна (списки рисуются из сообщения Host, «Создать…» последним
  пунктом, меню поверх содержимого); создание мира (с общей кампанией, демо-квестом
  и папкой `Saves`) и кампании (файл + папки `quests`/`scenes`); окна «Ред.»/«ℹ️»
  для мира и кампании (изображение, полное имя, описание, автор и даты, состав);
  экспорт папкой либо архивом `.aqezip` (галочка, умолчание — папка); импорт
  кампании и квеста с выбором родителя; оранжевое предупреждение о чужом родителе.
  Все ветки проверяются прогонами по диску (10 новых smoke-проверок, CI: 44).
  НАЙДЕНО и исправлено прогонами: `CampaignStore` при `ScopedTo` писал кампанию
  рядом с `world.aqworld` вместо `campaigns/`; созданный мир не имел папки `Saves`;
  занятое имя архива проверялось только по файлу (папка с тем же именем ломала
  выгрузку); имя ресурса с разделителем проходило через `Path.GetFileName`.
