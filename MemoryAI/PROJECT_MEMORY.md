...
Основной барьер — локальный прогон `ci/run_local.ps1` набором `fast` (около минуты, 15 проверок). Он вызывается из `pull.ps1` до `compile.ps1`; при ошибке сборка не выполняется, а отчёт пишется в `MemoryAI/LOGS/CI_errors.md`.

Проверки разбиты на три набора: `fast` — обязательный гейт перед pull; `acceptance` — ручная приёмка перед выпуском (публикация, архив/импорт/экспорт, хранение мира, свойства ресурсов, создание, автосохранение, инвентарь, иконки, читаемость диалогов, заставка); `nightly` — тяжёлое и внерелизное (R32, ручные перекрёстки, визуализация Location, память). Набор задаётся флагом `-Suite`. Контроль мутаций R32 идёт около семи минут, поэтому вынесен из гейта: в гейте проверка, которую никто не ждёт, перестаёт быть гейтом. Группы smoke идут одним браузером на группу через `ci/lib/run_suite.mjs`, состав — `ci/suites.json`.
...
- Reputation — репутация по каждому НПЦ отдельно (`NpcReputationEntry`: значение и
  факт контакта), а не по фракциям: у каждого НПЦ своя шкала и свой портрет.
  Шкала, названия диапазонов и цвета считаются в `ReputationScale` одного места,
  чтобы UI не дублировал пороги.
...
- локальный `ci/run_local.ps1` покрывает Dialogue Workspace внутри группы `editor` (набор `fast`) и проверяет syntax `dialogueWorkspace.js`;
...
## 2026-09-23 — 1.0.40.181 Единое дерево миров (отменяет раскладку выше)

**Раздел 1.0.40.148 устарел.** Пользовательское хранилище переехало из
`%LOCALAPPDATA%\Assist Quest Editor\quests` в **Документы**, а дублирование
`data/` устранено: поставка больше не держит второй копии контента рядом с EXE.

Раскладка (контракт живёт в `Domain/WorldPaths.cs`, чтобы App и Domain читали одно):

```
Документы\Assist Quest Editor\
  worlds\
    <Мир>\
      world.aqworld            ← мир = проект
      worlds.image / world.png
      campaigns\
        <Кампания>\
          campaign.aqcampaign  ← кампания внутри мира
          quests\<Id>.aqquest  ← квест внутри кампании
          scenes\
      Saves\                   ← автосохранения СВОЕГО мира
      Exported\                ← выгрузки
```

Почему сохранения внутри мира: снимок одного мира нельзя загрузить в другой, где
другие квесты и точки.

**Идентичность ресурса.** У мира и у кампании есть собственные `created_by`,
`created_on`, `modified_by`, `modified_on` (`ResourceMetadata`), а также полное имя,
описание и изображение. Кампания — не «список квестов», а ресурс: у неё свой id
(из имени папки), свой файл и свои даты. Правила подписи — `WorldDisplayRules` в
домене: их читают список, селектор и импорт, и разойтись формулировки не могут.

**Экспорт.** Папкой либо архивом `.aqezip` (галочка «Архивация в aqezip», по
умолчанию — папка). Архив — `ZipArchive` с `CompressionLevel.SmallestSize` и
ФИКСИРОВАННЫМ временем записи элементов: иначе байты менялись бы от прогона к
прогону, и `ci/archive_smoke.mjs` не мог бы сверять файл побайтово. Папки в ZIP
кладутся явными записями — формат хранит только файлы, и пустые каталоги (`Saves`)
иначе терялись бы. Имя ресурса с разделителем, `..` или `:` отвергается явно:
`Path.GetFileName("../x")` вернул бы `"x"`, то есть «санитайзер» молча сменил бы
место записи вместо отказа.

**Импорт.** Двойной клик по `.aqezip` запускает приложение и открывает диалог
импорта: состав архива показывается ДО распаковки. Родитель выбирается явно (в
архиве он может быть указан, но может и не существовать здесь). Кампания
распаковывается в `campaigns/<id>/`, квест — ОДНИМ файлом в `quests/<id>.aqquest`
(не папкой).

**Чужой родитель — предупреждение, не запрет.** Папку можно перенести файловым
менеджером, и перенос мог быть осознанным. `ResourceParentRules.IsForeignParent`
(пустой родитель — НЕ расхождение, иначе весь старый контент объявился бы чужим)
даёт оранжевую плашку с объяснением. Без неё квесты чужой кампании выглядят
пропавшими, а причина не видна вовсе.

**Смена мира требует перезапуска.** Селектор сохраняет выбор и просит перезапустить
приложение вместо молчаливой смены заголовка: пересоздание сторов, `QuestRuntime`,
