...
CI запускается при push в main, поэтому каждый отдельный commit в main является отдельным запуском полного workflow.

Для ускорения диагностики используется кумулятивная схема:

1. связанные изменения одной задачи накапливаются вместе;
2. доступные локальные проверки выполняются до push;
3. готовый пакет отправляется одним commit в main;
4. запускается один полный CI-проход;
5. после результата CI создаётся только один следующий исправляющий commit, если ошибка найдена.
...
Workflow дополнительно использует отмену устаревших незавершённых запусков через concurrency, чтобы случайный повторный push не оставлял несколько конкурирующих CI-проходов.
