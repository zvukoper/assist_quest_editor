...
Перекрёстки — тоже отдельный ресурс (`data/world/junctions.json`, 61.8 КБ, 3 483 узла), но, в отличие от дорог, они приходят ГОТОВЫМ списком, а не считаются в рантайме: поиск узлов требует нодировки всей сети (рассечение 98 341 отрезка во всех пересечениях) — это сотни миллисекунд и десятки мегабайт промежуточных структур на каждый запуск. Домен читает файл через `JunctionWorldDataLoader` и строит `JunctionIndex` (клетки 250 м — узлы точечные, но в центре крупного города их до 38 на клетку 2×2 км; у дорог клетка 500 м, потому что отрезки длинные).

Сеть дорог НЕ разбита в точках пересечения, поэтому «взять концы отрезков» не работает: из 133 596 различных концов 70 562 (52.8%) — тупики, а узлов степени ≥3 по точному совпадению концов всего 49 на всю карту. Построитель `ci/import_junctions.mjs` делает пять шагов, и два из них обязательны:

1. **Нодировка** — отрезки режутся во всех настоящих пересечениях (6 426 точек).
2. **Сшивка «конец → линия»** (20 м) — конец примыкающей улицы упирается в КРАЙ проезжей части (~13 м ширины), а не в ось, поэтому проецирование на чужую дорогу обязательно; без него Т-образных узлов 326 вместо 677.
3. **Схлопывание близких направлений** (30°) — 6% отрезков имеют параллельного двойника на медиане 13 м; без схлопывания сложных узлов 3.4% вместо 0.6%, потому что каждый поворот двухполосной дороги даёт лишние ветки.
4. **Узел** — ≥3 РАЗЛИЧНЫХ направлений; углы считаются полные (0..2π), а не по модулю π, иначе сквозная дорога (дающая два направления) неотличима от Т-образного примыкания.
...
**Окно ручной проверки** (`JunctionReviewForm`, `junctions.html`, `junctions.js`) нужно потому, что автомат неизбежно ошибается: ложные перекрёстки надо исключать руками, а пропущенные — добавлять. Окно рисует дороги и названия городов (точки мира НЕ показываются), найденные узлы — жёлтыми точками, навигация «следующий/предыдущий» идёт в порядке, в котором узлы нашлись (`JunctionIndex.ToPoints()` отдаёт список в исходном порядке), есть мультивыбор Ctrl+кликом. «Исключить» откладывает выбранные узлы, «Вернуть» возвращает. **Клик по пустому месту создаёт зелёную точку** — пропущенный перекрёсток; повторный клик её убирает.
